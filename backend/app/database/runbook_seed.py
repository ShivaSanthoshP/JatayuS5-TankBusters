"""Canonical runbook seeding — shared by the CLI script and app startup.

`seed_canonical_runbooks()` upserts the canonical set of issue-type runbooks
into the database and mirrors them into the vector store for RAG retrieval.
...
This lives in the app package (not `scripts/`) so it can be imported from the
application lifespan and therefore runs *inside* the uvicorn process — using
the same `DATABASE_URL` the service is configured with. Seeding from an
out-of-process shell (e.g. a deploy step) silently targets the default SQLite
DB when `DATABASE_URL` is only provided via systemd, which is how the seed
previously landed in the wrong database.
...
`{node_name}`, `{prefix}`, `{issue_type}` placeholders that the runtime agents
render at lookup time with concrete incident values.
"""
...
logger = logging.getLogger("itops.runbook_seed")
...
def _all_canonical_present(db) -> bool:
    """True when every canonical issue_type already has a seeded row."""
    seeded_types = {
        t for (t,) in db.query(RunbookEntry.issue_type)
        .filter(RunbookEntry.is_seeded.is_(True)).all()
        if t
    }
    return set(ISSUE_PROFILES).issubset(seeded_types)


def seed_canonical_runbooks(*, only_if_missing: bool = False) -> dict:
    """Upsert the canonical runbooks into the DB and vector store.

    When `only_if_missing` is set (used on app startup), this is a no-op once
    every canonical issue_type is already seeded — so it self-heals a fresh DB
    without re-writing on every boot.
    """
    init_db()

    counts = {"created": 0, "updated": 0, "skipped": False}
    db = SessionLocal()
    try:
        if only_if_missing and _all_canonical_present(db):
            logger.info("Canonical runbooks already present — skipping seed")
            counts["skipped"] = True
            return counts
        ...
        for issue_type, profile in ISSUE_PROFILES.items():
            ...
            # Mirror into the vector store for RAG retrieval (best-effort).
            try:
